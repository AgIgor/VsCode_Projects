...
// A2: LDR simulado por potenciometro

const uint8_t IN_LOCK = 2;
const uint8_t IN_TURN_SIGNAL = 3;
const uint8_t IN_DOOR = 4;
const uint8_t IN_IGNITION = 5;
const uint8_t IN_UNLOCK = 6;
...
DebouncedInput inLock = {IN_LOCK, HIGH, HIGH, 0};
DebouncedInput inTurn = {IN_TURN_SIGNAL, HIGH, HIGH, 0};
DebouncedInput inDoor = {IN_DOOR, HIGH, HIGH, 0};
DebouncedInput inIgnition = {IN_IGNITION, HIGH, HIGH, 0};
DebouncedInput inUnlock = {IN_UNLOCK, HIGH, HIGH, 0};
...
void setup() {
  ...
  debugLog(now, F("Debug serial habilitado para Arduino Uno com LDR em A2."));
  debugLogEvent(
      now,
