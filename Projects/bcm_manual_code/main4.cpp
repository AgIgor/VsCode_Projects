...
statusCarro carroAnterior = {false, false, false, false, false, false, false, false, false, false};
...
void atalizaCarroAnterior(){
  carroAnterior.ignicao = carro.ignicao;
  carroAnterior.porta = carro.porta;
  carroAnterior.trava = carro.trava;
  carroAnterior.destrava = carro.destrava;
  carroAnterior.farol = carro.farol;
  carroAnterior.drl = carro.drl;
  carroAnterior.luzTeto = carro.luzTeto;
  carroAnterior.luzPe = carro.luzPe;
  carroAnterior.saiu = carro.saiu;
  carroAnterior.entrou = carro.entrou;
}
...
void processaStatus(){
  ...
  if(carro.ignicao == false and carro.porta == false and carro.trava == true and carroAnterior.ignicao == true){ //se ignição era ligada e agora está desligada, porta fechada e trava ativada, desligou motor, ligar farol por 1s e desligar após 10s
    T_on_farol.start(1000); //ligar farol após 1s
    T_off_farol.start(10000); //desligar farol após 10s 
  };
  
  if(carro.ignicao == false and carro.porta == false and carro.destrava == true and carroAnterior.ignicao == false){ //se ignição era desligada e ainda está desligada, porta fechada e destrava ativada, travou,  ligar farol por 2s e desligar após 6s
    T_on_farol.start(2000); //ligar farol após 2s
    T_off_farol.start(6000); //desligar farol após 6s
  };

  printStatus();
  atalizaCarroAnterior();
}
...
void lerEntradas(){
  for (int i = 0; i < totalEntradas; i++) {
    ...
    if(entradas[i].estadoAnterior != entradas[i].estadoAtual){
      registrarEvento(i, entradas[i].estadoAtual);
      Serial.print("Entrada ");
      Serial.print(i);
      Serial.print(" mudou para ");
      Serial.println(entradas[i].estadoAtual ? "HIGH" : "LOW");
